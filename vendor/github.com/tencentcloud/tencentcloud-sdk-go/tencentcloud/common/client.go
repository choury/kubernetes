...
type Client struct {
	...
	credential      CredentialV3Interface
	signMethod      string
...
func (c *Client) sendWithSignatureV3(request tchttp.Request, response tchttp.Response) (err error) {
	...
	// sign string
	skey, err := c.credential.GetSecretKey()
	if err != nil {
		return err
	}
	secretDate := hmacsha256(date, "TC3"+skey)
	...
	// build authorization
	sid, err := c.credential.GetSecretId()
	if err != nil{
		return err
	}
...
func (c *Client) WithCredential(cred CredentialV3Interface) *Client {
	c.credential = cred
	return c
}
