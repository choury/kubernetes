...
func signRequest(request tchttp.Request, credential CredentialV3Interface, method string) (err error) {
	if method != SHA256 {
		method = SHA1
	}
	err = checkAuthParams(request, credential, method)
	if err != nil {
		return
	}
	s := getStringToSign(request)
	skey, err := credential.GetSecretKey()
	if err != nil {
		return
	}
	signature := Sign(s, skey, method)
	request.GetParams()["Signature"] = signature
	return
}

func checkAuthParams(request tchttp.Request, credential CredentialV3Interface, method string) error {
	params := request.GetParams()
	credentialParams, err := credential.GetCredentialParams()
	if err != nil {
		return err
	}
